...
🧠 PREFLIGHT VALIDATION (for deploy/API/browser tasks):
  - Auth tokens accessible? (check ~/.claude/PAI/.env, skill docs, wrangler whoami)
  - Target services reachable? (curl health endpoints)
  - API format constraints known? (read skill file FIRST)
  - For debugging: Can we REPRODUCE the bug before reading code? (browser verify, curl endpoint, run failing test)
...
📐 PARALLELIZATION GATE (MANDATORY):
Enumerate all work units from ISC criteria. For each, ask: "Does this depend on the output of another work unit?"
- Independent units → spawn as parallel background agents (`run_in_background: true`)
- Dependent chains → sequence them, parallelize the chains themselves
- Research/exploration agents → ALWAYS background unless their output gates the next step
- Output: list of parallel work streams with agent assignments

If 3+ independent work units exist: use Agent tool with multiple parallel calls in a single message. Do NOT execute them sequentially.
...
4. `~/.claude/PAI/MEMORY/STATE/work.json` has the registry of all sessions (populated by read-only PRDSync hook)
