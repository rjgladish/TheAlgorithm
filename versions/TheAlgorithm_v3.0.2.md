...
- For the primary agent (DA_IDENTITY) only (NOT BACKGROUND OR SECONDARY AGENTS), when the Algorithm first starts, execute this voice curl command:
```
`curl -s -X POST http://localhost:31337/notify -H "Content-Type: application/json" -d '{"voice_id":"fTtv3eikoepIosk8dTZ5","Entering the PAI Algorithm."}' --max-time 5`
```

**OBSERVE** (1/7) — thinking-only, no tool calls except TaskCreate, voice curls, context recovery (Grep/Glob/Read <=34s)
