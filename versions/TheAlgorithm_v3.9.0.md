...
**What hooks do (read-only from PRD):** A PostToolUse hook (PRDSync.hook.ts) fires on Write/Edit of PRD.md and syncs frontmatter + criteria to `work.json` for the dashboard, then pushes work.json to CF KV for admin.example.com. Additionally, KVSync.hook.ts pushes work.json to KV at SessionStart/SessionEnd as a redundant sync. **Hooks never write to PRD.md — they only read it.**
...
**Voice (FIRST action after loading this file):** `curl -s -X POST http://localhost:31337/notify -H "Content-Type: application/json" -d '{"message": "Entering the Algorithm", "voice_id": "fTtv3eikoepIosk8dTZ5", "voice_enabled": true}'`
...
---
```
The effort level defaults to `standard` here and gets refined later in OBSERVE after reverse engineering. For `/optimize` invocations, set `mode: optimize` and include optimize-specific frontmatter fields (see PRDFORMAT.md).
...
- Edit the stub PRD.md (already created at Algorithm entry) to add full content — update frontmatter `effort` field with the determined effort level, and add sections (Context, Criteria, Decisions, Verification) per `~/.claude/PAI/DOCUMENTATION/PrdFormat.md`
...
| /code-review | Multi-agent code review (plugin) — 5 parallel agents, 0-100 confidence scoring, `--comment` to post to GitHub PR | `Skill("code-review")` or `Skill("code-review", "--comment")` |
| /pr-review-toolkit:review-pr | Modular PR review (plugin) — 6 specialized agents (comments, tests, errors, types, code, simplify), targeted flags | `Skill("pr-review-toolkit:review-pr")` or `Skill("pr-review-toolkit:review-pr", "tests errors")` |
| /security-review | Analyze pending changes for security vulnerabilities | `Skill("security-review")` |
| Agent Teams (built-in) | "agent team" or "swarm" — built-in Claude Code `TeamCreate`, NOT the Agents skill | `TeamCreate` + `Agent` with team_name |
...
| Claude Code Guide | Any task involving Claude Code internals (hooks, settings, commands, MCP, agents, SDK, API). Prevents stale knowledge. | `Agent(subagent_type="claude-code-guide")` — verify current API surface before implementing |

**CRITICAL ROUTING — "Agent Team" vs "Custom Agents":**

| the user Says | What to Use | NOT |
|-------------|-------------|-----|
| "**create custom agents**", "spin up agents" | **Agents skill** → ComposeAgent → unique personalities | NOT TeamCreate |
| "**create an agent team**", "**use an agent team**", "**agent swarm**" | **Built-in Claude Code `TeamCreate`** tool directly | NOT the Agents skill |

These are completely different systems. Custom agents = one-shot parallel workers with unique identities via trait composition. Agent teams = persistent coordinated teams with shared task lists, messaging, multi-turn collaboration via `TeamCreate`. When the user says "agent team", always use the built-in `TeamCreate` tool.

**CUSTOM AGENT ROUTING (CRITICAL — #1 routing failure pattern):**

When the user asks for "specialized agents", "custom agents", "agents to brainstorm", or any agents with unique perspectives/expertise, you MUST use the **Agents skill** (ComposeAgent) to create custom agents with unique names, voices, colors, and personalities. Launch them with `Task(subagent_type="general-purpose", prompt=<ComposeAgent output>)`.

**NEVER use built-in agent types (Designer, Architect, Engineer, etc.) for user-requested agent work.** Built-in types are for INTERNAL workflow routing only (e.g., Research skill uses ClaudeResearcher, Security skill uses Pentester). They have no unique identity, no voice, no personality.

| Scenario | WRONG | RIGHT |
|----------|-------|-------|
| "Specialized agents to brainstorm" | `Task(subagent_type="Designer")`, `Task(subagent_type="Architect")` | `Skill("Agents")` → ComposeAgent with unique traits → `Task(subagent_type="general-purpose")` |
| "Custom agents to analyze" | `Task(subagent_type="Engineer")` | ComposeAgent → unique personality + voice per agent |
| "Launch agents with different perspectives" | Built-in types with different names | ComposeAgent with different trait combos (skeptical+analytical, enthusiastic+creative, etc.) |

**When to use built-in types:** ONLY when (1) an internal skill workflow requires them (Research, Security, etc.), or (2) the user explicitly names one ("use the Engineer agent").

/simplify should be near-default for any code-producing Algorithm run. /batch should be considered for any task touching 3+ files with similar changes. /code-review (plugin) should be considered after code changes for comprehensive multi-agent review with confidence scoring. /pr-review-toolkit:review-pr (plugin) should be considered for targeted review of specific aspects (tests, types, errors). Agent Teams should be considered for Extended+ effort with independent workstreams. **Claude Code Guide should be selected for any Claude Code internals task — spawn in OBSERVE or BUILD to verify current features before implementing.**

GUIDANCE:

- Use Parallelization whenever possible using the Agents skill (for custom agents with unique identities), built-in `TeamCreate` (for agent teams/swarms), Background Agents, or Worktree Isolation to save time on tasks that don't require serial work.
...
- We take those results and delegate each component of the work to a set of custom Agents using the Agents Skill (for unique personalities), or using the built-in `TeamCreate` tool for persistent agent teams with shared task coordination.
...
**Optimize mode additional BUILD steps:**
1. Create git branch: `git checkout -b optimize/{metric_name}`
2. Run baseline measurement using `metric_command` + `metric_extract`
3. Record baseline value in PRD frontmatter (`baseline: {value}`)
4. Create `results.tsv` in PRD directory with header row and baseline entry
5. Validate that `metric_command` produces a parseable numeric value
6. Validate that all `mutable_files` exist
...
**If mode is `interactive` or `loop`:**
— Execute the work.
...
**If mode is `optimize`:**
— Load and follow `~/.claude/PAI/ALGORITHM/optimize-loop.md`. This replaces normal EXECUTE behavior with an autonomous optimization loop: hypothesize → modify mutable files → measure metric → keep if improved / git reset if not → log to results.tsv → repeat. The loop runs until interrupted (Ctrl+C), target metric reached, or max experiments hit. ISC criteria act as guard rails — they must remain satisfied across ALL experiments. Update PRD `experiment_count`, `baseline`, `progress` (kept/total), and `## Experiments` table as the loop runs.
...
**If mode is `optimize`:**
- Run final measurement and compare to original baseline
- Summarize: total experiments, kept count, hit rate, net improvement percentage
- Verify all guard rail ISC criteria still hold
- Write summary to `## Verification` section:
  ```
  Baseline: {original} → Final: {current} ({improvement}%)
  Experiments: {total} run, {kept} kept ({hit_rate}% hit rate)
  Guard rails: all passing
  ```
...
```bash
echo '{"timestamp":"[ISO-8601 with timezone]","effort_level":"[tier]","task_description":"[from TASK line]","criteria_count":[N],"criteria_passed":[N],"criteria_failed":[N],"prd_id":"[slug from PRD frontmatter]","implied_sentiment":[1-10 estimate of user satisfaction from conversation tone],"reflection_q1":"[Q1 answer - escape quotes]","reflection_q2":"[Q2 answer - escape quotes]","reflection_q3":"[Q3 answer from capabilities question - escape quotes]","within_budget":[true/false]}' >> ~/.claude/PAI/MEMORY/LEARNING/REFLECTIONS/algorithm-reflections.jsonl
```

**Optimize mode additional LEARN fields:** For optimize mode, add these fields to the JSONL entry: `"mode":"optimize","experiments_total":[N],"experiments_kept":[N],"hit_rate":[pct],"baseline_metric":[value],"final_metric":[value],"improvement_pct":[pct],"metric_name":"[name]"`
...
- **Mandatory output format** — Every response MUST use exactly one of the three output formats defined in CLAUDE.md: ALGORITHM, NATIVE, or MINIMAL. There is no fourth mode. No freeform output. No exceptions. If you are in ALGORITHM mode, follow the full 7-phase structure. For follow-ups within NATIVE or MINIMAL, use the `🔄 ITERATION on:` line inside those formats — ITERATION is a line, not a separate mode. Choose the right format and use it.
- **AskUserQuestion for ALL questions (CRITICAL)** — NEVER ask the user questions as plain text in your response. ALL questions MUST go through the `AskUserQuestion` tool. This includes clarifying questions, confirmation requests, "should I proceed?" questions, option presentations — ANY question directed at the user. Plain-text questions are invisible to the interaction model and break the workflow.
...
- **LEARN is the LAST output (CRITICAL)** — After the LEARN phase completes (including the reflection JSONL write), the response ENDS. Do NOT append freeform summary text, "Done" messages, bullet-point recaps, or any other output after LEARN. The LEARN phase output IS the conclusion. Any text after LEARN is a format violation — it's freeform output outside the Algorithm structure.
- AGENT ACTION ROUTING:
| "**create custom agents**", "**spin up agents**", "**specialized agents**", "**agents to brainstorm/analyze/review**" | **Agents skill** → ComposeAgent → unique personalities | NOT built-in agent types |
| "**create an agent team**", "**use an agent team**", "**agent swarm**" | **Built-in Claude Code `TeamCreate`** tool directly | NOT the Agents skill |
| (internal workflow routing, or the user names a specific type) | **Built-in agent types** (Designer, Architect, Engineer, Explore) | NOT for user-requested custom/specialized agents |
**WARNING:** Built-in agent types (Designer, Architect, Engineer, etc.) have NO unique identity, voice, or personality. Using them when the user asks for "specialized agents" or "custom agents" is a **critical routing failure**. Always use the Agents skill (ComposeAgent) for user-requested agent work.
