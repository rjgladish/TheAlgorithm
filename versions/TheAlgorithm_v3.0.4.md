...
### IDEAL STATE Creation Rules

**System of record: PRD.md (direct write).** The AI writes PRD.md directly using Write/Edit tools in `MEMORY/WORK/{slug}/`. A ResponseComplete hook (PRDStateSync) reads PRD frontmatter and populates `MEMORY/STATE/work.json` as a derived registry for consumers (voice, dashboard, tabs). TaskCreate/TaskUpdate are optional for in-session UI display.
...
**OBSERVE** (1/7) — Edit PRD frontmatter `phase: observe`. Then thinking-only, no tool calls except TaskCreate, context recovery (Grep/Glob/Read <=34s)
...
- IDEAL STATE Criteria Generation (TaskCreate for each criterion)
...
- TaskCreate is optional for in-session sidebar UI display
...
**THINK** (2/7) — Edit PRD frontmatter `phase: think`. Pressure test and enhance the ISC:
...
**PLAN** (3/7) — Edit PRD frontmatter `phase: plan`. EnterPlanMode if EFFORT LEVEL is Advanced+.
...
**EXECUTE** (5/7) — Edit PRD frontmatter `phase: execute`. Perform the work.
...
- TaskUpdate is optional for sidebar UI.

**VERIFY** (6/7) — Edit PRD frontmatter `phase: verify`. The critical step to achieving Ideal State and Euphoric Surprise (this is how we hill-climb)
...
**LEARN** (7/7) — Edit PRD frontmatter `phase: learn`. After reflection, set `phase: complete`. Algorithm reflection and improvement
...
4. `~/.claude/PAI/MEMORY/STATE/work.json` has the registry of all sessions (populated by PRDStateSync hook)
