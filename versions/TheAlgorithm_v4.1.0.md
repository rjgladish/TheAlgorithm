...
### CHANGES FROM v4.0.1 → v4.1.0

v4.1.0 is a **vocabulary release** — no doctrine changes. The artifact previously called "PRD" (Product Requirements Document) is renamed to **ISA — Ideal State Artifact**. Three justifications, each load-bearing:

1. **Voice-flow.** ISA pronounces as a single English word ("EYE-suh"); PRD forces letter-by-letter robot speech. Daily quality-of-life for voice-rendered work.
2. **ISC pairing.** ISA holds the ISC. The artifact-vs-criteria split now reads as a single compositional story.
3. **Hard-to-vary.** "Artifact" implies tangible verifiable output — the engineering noun. The chosen name plays a functional role; varying it (Document, Spec, Brief) weakens the connection to verification.

**Filename change with alias period:** new sessions write `MEMORY/WORK/{slug}/ISA.md`. Hooks read both `ISA.md` and `PRD.md` for one minor version (v4.1.x). At v4.2.0, `PRD.md` fallback is removed.

**JSONL field rename with alias:** `prd_level_audit` → `isa_level_audit`. Both fields written for one minor version.

**Type rename with alias:** `PrdFrontmatter` → `IsaFrontmatter` in `PULSE/Observability/src/types/algorithm.ts`. Deprecated type alias kept for one minor version.

**What did NOT change:** seven phases, voice announcements, PRD→ISA frontmatter required fields, PLA, per-ISC HVA, all v4.0 doctrine, RR1/RR2/RR3, all phase boundaries, voice format, frontmatter contract, Forge/Anvil/Cato bindings, fast-path trigger. v4.0.1 in full is preserved — only the noun changes.
...
1. **`[A]` category parser fix (P0).** `~/.claude/hooks/lib/prd-utils.ts` had a hard-coded `VALID_CATEGORIES = Set(['F','S','B','N','E'])`. The `[A]` Antecedent category added in v3.30 R3 was being silently stripped at parse time, so `work.json` and Pulse never saw v4.0 PRDs (now ISAs) as having Antecedent ISCs. Added `'A'` to the set.
...
**The ISA is the unit, not the ISC.** v4.0.0 elevates this: per-ISC hard-to-variability is necessary but not sufficient. The ISA as a whole must satisfy three system-level tests:
...
This doctrine is not decoration. It is the criterion by which every gate, audit, and phase in this Algorithm is justified. When a rule below seems heavy, ask whether removing it would let the ISA vary without destroying the goal — and if it would, the rule earns its place.

---

### CHANGES FROM v3.30.0 → v4.0.0

v4.0.0 is a **frame-shift release**, not a patch. v3.30.0 grounded ISCs in hard-to-vary doctrine — load-bearing per line. v4.0.0 elevates the unit of analysis: **the PRD (now ISA) itself is the hard-to-vary explanation of the ideal state**, in Deutsch's sense. ISCs are decomposed components. This matters because per-ISC HVA cannot see three failure modes:
...
- **P4 — Durable-deliverable concrete binding (Rule 2)** — Any `phase: complete` transition on an Extended+ effort PRD (now ISA) = MANDATORY advisor call. No self-assessment of "not yet durable" — the Extended+ tier is the binding. Closes the floating-goalpost escape where multiple small deliverables could all be classified as "not durable yet."
...
**Dual-source phase tracking (since 2026-04-16; extended 2026-04-18):** Both voice (via this curl) and ISA frontmatter edits feed `phaseHistory` in `work.json`. Voice creates timeline entries with `source: "voice"`; ISA edits upgrade them to `source: "merged"` when both fire. Missing a voice call still records the phase (ISA catches it); missing an ISA edit still records the phase (voice catches it). See `hooks/lib/isa-utils.ts::appendPhase()` for dedup logic.

**2026-04-18 fix — voice lane now also updates top-level `session.phase` AND terminal tab icon via `setPhaseTab()`**, closing the gap where voice-only transitions left `/agents` UI and kitty tab stuck at the last ISA-edited phase. Consumers of top-level `phase` (Pulse `/api/observability/state`, `/agents` dashboards, kitty tab titles) now reflect voice transitions without requiring an ISA edit round-trip. Per-session kitty socket persistence lives at `MEMORY/STATE/kitty-sessions/{sessionUUID}.json`, written by `KittyEnvPersist.hook.ts` at SessionStart. The `tab-setter.ts` utility resolves the `kitten` binary via `command -v` with a fallback to `/Applications/kitty.app/Contents/MacOS/kitten` so the Pulse daemon (launchd-restricted PATH) can also invoke it; all `kitten @` calls now include `--match="id:{windowId}"` to target the correct tab even when called from an unfocused process.

### ISA as System of Record

ISA.md in `~/.claude/PAI/MEMORY/WORK/{slug}/` is the single source of truth. The AI writes ALL content directly. Hooks only read the ISA.

**Filename alias note (v4.1.x).** New sessions write `ISA.md`. Hooks read both `ISA.md` and the legacy `PRD.md` filename for the duration of the v4.1.x line — if a session directory contains a `PRD.md` and no `ISA.md`, hooks fall back to `PRD.md` so in-flight work from v4.0.x is not orphaned. At v4.2.0 the `PRD.md` fallback is removed and any remaining `PRD.md` files must be renamed before that release. New work should never create a `PRD.md`; this alias exists only to keep already-running sessions intact across the rename.

**Frontmatter:** `task`, `slug`, `effort`, `phase`, `progress`, `mode`, `started`, `updated`. Optional: `iteration`, `algorithm_config`.
**Body:** `## Context`, `## Criteria`, `## Decisions`, `## Verification`. Full spec: `~/.claude/PAI/DOCUMENTATION/ISA/IsaFormat.md`.
...
- `- [DEFERRED-VERIFY]` — passed in code/intent but live probe is impossible at execution time (long async deploys, third-party services without test endpoints, feature-flagged paths). **Requires a follow-up task ID in the verification notes.** Cannot be marked `[x]` until the deferred probe runs. Counts as "not yet complete" for progress tracking — an ISA with any `[DEFERRED-VERIFY]` items cannot reach `phase: complete` unless the deferred probes are explicitly waived in `## Decisions` with reason. (v3.24 P3)

### Tunable Parameters

Modes (ideate, optimize) accept tunable parameters. Full schema and presets: `~/.claude/PAI/ALGORITHM/parameter-schema.md`. Parameters stored in ISA `algorithm_config:` frontmatter.
...
**Why this exists:** Analysis of 319 failures in April 2026 showed 31% were caused by intent drift — the Algorithm startup ceremony (voice, ISA, capability scan) consumed attention and the original request was lost. This echo creates a token-level anchor that survives the ceremony.
...
1. Check for explicit E-level override (`/e1`-`/e5` or `E1`-`E5`, case-insensitive, standalone token) in user message
   - If found: use corresponding tier, set `effort_source: explicit` in ISA frontmatter
...
**Write ISC criteria** directly into ISA with category tags:
...
| Testability | Every criterion must be tool-verifiable (not subjective); max 2 per ISA require human judgment | Rewrite vague criteria to be specific and measurable |
...
### 🧬 ISA-LEVEL HARD-TO-VARY AUDIT (NEW in v4.0 — ILA)

**The ISA is the unit. Audit the system before auditing the lines.**

Required at E2+; one-line note acceptable at E1. Runs **before** the per-ISC HVA below — per-ISC fluff inside a fundamentally broken ISA is wasted work.

Three tests applied to the ISA's complete ISC set as a system:
...
Any non-`tight` finding requires ISA-level surgery — add the missing facet, remove the redundant subset, sharpen the goal phrasing — before per-ISC HVA proceeds. **The ISA is what is being explained; the ISCs are how it is explained.** A v4.0 ISA only enters the per-ISC HVA after the system passes.

**Why this exists:** v3.30.0's per-ISC HVA caught line-level fluff but was blind at the system level. An ISA with 30 perfectly load-bearing ISCs aimed at the wrong (or incomplete, or fuzzy) goal would pass v3.30 doctrine and ship the wrong thing. ILA tests the explanation as a whole — which is what Deutsch's hard-to-variability is actually about. Single-line tests are a special case.
...
Evidence format in ISA `## Verification`:
```
...
**Durable-deliverable concrete binding (v3.24 P4):** For Extended+ effort ISAs, the `phase: complete` transition IS the durable-deliverable moment. No self-assessment of "not yet durable" — the effort tier is the binding. Any Extended+ ISA heading into LEARN's `phase: complete` MUST invoke the advisor at least once. This closes the floating-goalpost escape where executors classified their deliverables as "not yet durable" to avoid the call.
...
bun ~/.claude/PAI/TOOLS/Inference.ts --mode advisor \
  ...
  "STATE: ISA slug, ISC progress, relevant decisions, recent tool outputs" \
  "QUESTION: specific decision point or 'any gaps before declaring done?'"

# v3.24 P5: Auto-synthesized state (reads ISA + recent tool outputs automatically)
bun ~/.claude/PAI/TOOLS/Inference.ts --mode advisor --auto-state \
  "TASK: one-sentence description" \
  "QUESTION: specific decision point"
# (state is omitted — helper reads current ISA verbatim + recent tool activity)
```
...
const review = await advisor({
  ...
  autoSynthesize: true,  // v3.24 P5: reads current ISA automatically
});
```
...
**v3.24 P5 — auto-synthesized state:** When `autoSynthesize: true` (or `--auto-state` on CLI), the helper reads the current ISA and recent tool outputs directly instead of relying on the caller to write a state summary. This closes the biggest RedTeam Flaw: **the caller can no longer omit problem areas from what the reviewer sees**. Auto-synthesis is recommended for any commitment-boundary call where the state could plausibly be gamed — which is most of them. Manual state remains supported for cases where the caller has context the ISA doesn't capture.
...
```
===== ISA =====
<full contents of MEMORY/WORK/{slug}/ISA.md>

===== OUTPUT ARTIFACTS =====
<files referenced in ISA ## Decisions, up to 30K tokens>
...
- Rule 2a SKIPS only if `codex exec` is unavailable (CLI missing, API down). Log the skip with reason to `cato-findings.jsonl` as `{"skipped": true, "reason": "..."}`. Do NOT mark ISA complete without Rule 2a unless skipped for infrastructure reasons.
...
- **Inline Verification check (v3.26 T2):** Scan ISA `## Verification` for any ISC marked `[x]` without tool-probe evidence. Any found = CRITICAL FAILURE; re-probe before allowing LEARN.
...
**Note in ISA `## Verification`** what was captured (or SKIP), but the substance lives in KNOWLEDGE/, not the ISA.
...
🧬 ISA: [tight | gap | over | under — required at E2+ per v4.0 ILA]
