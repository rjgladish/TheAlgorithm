...
**Format:** `- [ ] ISC-N: criterion text` — the criterion phrasing reveals its category to any competent reader, no bracketed letter is required. *(provenance: v5.3.0 R1 — BPE compaction; the four descriptive tags `[F]`/`[S]`/`[B]`/`[E]` carried zero load and the two doctrinal gates were already encoded by other surface conventions.)*

**Two doctrinal ISC kinds preserved as prefix conventions:**

| Kind | Surface form | Rule |
|------|--------------|------|
| **Anti-criterion** — must NOT happen, no regressions | `- [ ] ISC-A-N: Anti: <what must NOT happen>` | **≥1 required** — a goal with zero failure modes worth naming is under-specified |
...
The `ISC-A-N` numbering and the `Anti:` / `Antecedent:` prose prefixes are the only surface signals the doctrine still depends on. Everything else is the criterion text doing the work.
...
```
♻︎ Entering the PAI ALGORITHM… (v5.3.0) ═════════════
