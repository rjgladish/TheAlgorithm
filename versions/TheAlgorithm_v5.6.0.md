...
**PAI moves people from current state to ideal state by writing down what done looks like as testable claims, then refining the writing until every claim survives every test it can be subjected to.** That is the master loop. The same primitive applies in any domain — code, science, art, philosophy, life decisions, business strategy. Every Algorithm run instantiates this loop on a specific task.

**Hard-to-variability is the quality standard for the writing.** David Deutsch's epistemology: knowledge is hard-to-vary explanation — a description where every detail plays a functional role, so contrary evidence has nowhere to flee. The ISA is the written hard-to-vary explanation of the ideal state for one task. A good ISA cannot be varied without destroying the goal; a bad ISA has fluff that could be weakened without changing the outcome.

**Testability is hard-to-variability made operational.** They are the same property described from operational and epistemic angles. An ISC is hard-to-vary if and only if you can name a test that would falsify it. The granularity rule ("split until each ISC is one binary tool probe") is how the doctrine enforces hard-to-variability concretely. Verification is the scientific method applied to ISCs — hypothesis (ISC), experiment (probe), result (pass/fail), refine.

**The ISA is a living explanation.** Hard-to-variability is the **outcome** of the algorithmic process, not its precondition. The ISA at OBSERVE captures the best initial articulation; through pursuit — feedback, tool returns, capability outputs, ISC failures, new signal — the Goal sharpens, ISCs split or merge, the explanation tightens. Every phase potentially refines it. Refinements are logged in `## Decisions` with a `refined:` prefix; git history of the ISA file is the trail. "Done" includes "the explanation became hard-to-vary enough that we know we hit it" — not just checkbox completion.
...
These tests **recur at phase boundaries** — not a one-time gate at THINK. New signal at any phase may surface a Coverage gap or Tightness violation; the ILA re-runs and the ISA refines. ISCs are how the explanation is decomposed into testable claims; the ISA is what is being explained.

**Opacity → transparency is the design move.** Vague human intent enters the system; the Algorithm climbs the writing toward something hard-to-vary — explicit, testable, falsifiable. The Goal paragraph (when present in the ISA per IsaFormat v2.5) is the tightest verbal form; the ISC set is its decomposition; verification is the scientific method that confirms the explanation held up against reality.

**The experiential metric is euphoric surprise** — what the user feels when a hard-to-vary explanation meets novelty: an answer that clicks in a way they couldn't have predicted but instantly recognize as true. Euphoric surprise is **not a separate optimization target** from hard-to-variability. They are the same phenomenon viewed from inside vs. outside: the *epistemic* test of a good explanation is hard-to-variability; the *experiential* test of encountering a good explanation is euphoric surprise. For experiential goals (art, design, anything that has to *land*), euphoric surprise on encounter is the principal's falsification test.

**Core loop:** current state → ideal state, with the ISA as the living hard-to-vary explanation of done, ISCs as testable claims that decompose it, verification as the scientific method that confirms it, refinement as the writing tightening through pursuit. **Goal: euphoric surprise on convergence.**
...
```
♻︎ Entering the PAI ALGORITHM… (v5.6.0) ═════════════
...
**ILA recurs at phase boundaries (v5.6.0).** Not a one-time gate at THINK. Re-run when new signal lands that could surface a Coverage gap or invalidate a Tightness/Uniqueness call: after a research/tool return reveals an unaccounted facet, after BUILD discovers a constraint not captured at OBSERVE, before VERIFY when a `refined:` decision was logged this run, at LEARN as a final sanity check. The ISA is a living explanation; the audit lives wherever the explanation is potentially mid-tightening.

| Test | Question | Failure mode |
|------|----------|--------------|
| **Coverage** | Does every facet of "done" (named in `## Goal` if present) map to ≥1 ISC? | Gap — a perfectly load-bearing line set that nevertheless leaves the goal incomplete |
...
Any non-`tight` finding requires ISA-level surgery — add the missing facet, remove the redundant subset, sharpen the Goal phrasing — and the change is logged in `## Decisions` with a `refined:` prefix. Per-ISC HVA proceeds against the refined ISA. *(provenance: v4.0 introduced ILA; v5.6.0 made it recurring rather than one-time, recognizing the ISA as a living explanation.)*
...
- `~/.claude/<harness-memory-dir>/` — harness auto-memory dir; blocked by `settings.json` permissions deny + `autoMemoryEnabled: false`. The router never routes here.
- Anywhere outside `~/.claude/` for PAI doctrine — public repos are scrubbed via `_PAI` release workflow only.
...
*(provenance: 2026-04-26 harness auto-memory removal + the user's directive to "incorporate this whole learning concept into the learn phase." Doctrine table mirrors PAI_SYSTEM_PROMPT.md "Self-Healing Infrastructure" so router and constitution agree.)*
...
The legacy `prd_level_audit` field (kept through v5.0.x for dashboard compatibility) is removed at v5.6.0. Dashboards should consume `isa_level_audit` only. v5.6.0 also adds `living_doc_refinements: [N]` — count of `refined:` entries written to the ISA `## Decisions` section during the run, instrumenting the living-document property empirically.
