...
```
♻︎ Entering the PAI ALGORITHM… (v3.16.0) ═════════════
...
**Knowledge capture** — flag knowledge worth archiving for the daily harvester. Write a `## Knowledge` section in the PRD.

OUTPUT:
```
📚 KNOWLEDGE UPDATE:
 📚 NEW: <domain>/<slug> — <8-word description of what was learned>
 📚 UPDATED: <domain>/<slug> — <8-word description of what changed>
 📚 SKIP — nothing worth archiving this session
```

**Rules for knowledge flagging:**
- Only flag genuinely reusable knowledge — patterns, decisions, gotchas, research findings
...
```markdown
## Knowledge
- NEW technology/cloudflare-d1-binding-pattern — Learned D1 binding requires explicit migration step
- UPDATED projects/surface — Cache invalidation requires purge after deploy, not before
```

If nothing worth archiving: write `## Knowledge\n- SKIP` and move on. Most sessions will SKIP — that's correct behavior.
