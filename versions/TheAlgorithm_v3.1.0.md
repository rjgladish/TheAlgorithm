...
**System of record: PRD.md (direct write).** The AI writes PRD.md directly using Write/Edit tools in `MEMORY/WORK/{slug}/`. PostToolUse hooks (PRDSync.hook.ts) provide real-time sync:

- **TaskCreate** → hook appends criterion to PRD `## Criteria` if missing, updates `progress:` counter, syncs to work.json
- **TaskUpdate (completed)** → hook checks box in PRD `## Criteria`, updates `progress:` counter, syncs to work.json
- **Write/Edit on PRD.md** → hook reads frontmatter, syncs to work.json immediately, announces phase changes

A ResponseComplete hook (PRDStateSync) runs at Stop as backup sync guarantee.

**Task ID = ISC number.** Task #1 = ISC-1, Task #2 = ISC-2. This mapping is deterministic — no fuzzy text matching.

**Domain separation:** AI owns prose content (Context, Risks, Plan, Decisions, Verification, Changelog) and phase transitions. Hooks own derived counters (progress) and work.json sync. Both can write criteria checkboxes with idempotent safety (hook checks before appending).
...
**OBSERVE** (1/7) — **FIRST ACTION:** Voice announce `"Entering the Observe phase."`, then Edit PRD frontmatter `phase: observe, updated: {timestamp}`. Then thinking-only, no tool calls except TaskCreate, context recovery (Grep/Glob/Read <=34s)
...
- IDEAL STATE Criteria Generation (TaskCreate for each criterion — **MANDATORY**, not optional. TaskCreate is the criteria intake channel. The PRDSync hook uses TaskCreate to append criteria to PRD and track progress.)
- Create PRD directory: `mkdir -p MEMORY/WORK/{slug}/` (slug format: `YYYYMMDD-HHMMSS_kebab-task-description`)
- Write PRD.md with Write tool — include YAML frontmatter (task, slug, effort, phase, progress, mode, started, updated) + standard sections (Context, Criteria, Risks, Plan, Decisions, Verification, Changelog)
- Add criteria as `- [ ] ISC-1: criterion text` checkboxes directly in the PRD's `## Criteria` section
- TaskCreate each criterion — this triggers the PRDSync hook to verify criteria are in the PRD
...
[POPULATE and show the TaskList with inital IDEAL STATE CRITERIA]
...
- If any CAPABILITIES were selected for use in the OBSERVE phase, execute them now and update the ISC TaskList with the results
...
- **WRITE TO PRD (MANDATORY):** Edit the PRD's `## Risks` section directly with each risk identified.
...
[Prerequisite validation. Update ISC using TaskUpdate if necessary. Reanalyze CAPABILITIES to see if any need to be added.]

- **WRITE TO PRD (MANDATORY):** Edit the PRD's `## Plan` section directly with technical approach, architecture, key decisions.
...
— Execute the work.
- As each criterion is satisfied, IMMEDIATELY edit the PRD: change `- [ ]` to `- [x]` and update frontmatter `progress:` field. Do NOT wait for VERIFY — update the moment a criterion passes.
- TaskUpdate(completed) for each passing criterion — this triggers the PRDSync hook to verify the checkbox and update progress.
...
— For EACH IDEAL STATE criterion (which are now your verification criteria) in the PRD, test that it's actually complete
...
- **WRITE TO PRD (MANDATORY):** Edit the PRD's `## Changelog` section directly with session summary — what was accomplished, what criteria passed, context for next session.
...
3. PRD body has criteria checkboxes, decisions, verification evidence, changelog
4. `~/.claude/PAI/MEMORY/STATE/work.json` has the registry of all sessions (populated by PRDSync + PRDStateSync hooks)
...
progress: 0/0
mode: interactive
started: 2026-02-23T13:40:02Z
updated: 2026-02-23T14:15:00Z
---
...
_What this task is, why it matters, what was explicitly requested and not requested._

## Criteria

- [ ] ISC-1: First criterion (8-12 words, binary testable)
- [ ] ISC-A-1: Anti: What must NOT happen

## Risks

_Riskiest assumptions, premortem findings, prerequisites._

## Plan

_Technical approach, architecture, key decisions, execution strategy._

## Decisions

- 2026-02-23 01:45: Chose X over Y because Z

## Verification

_Evidence for each criterion — what was tested, what passed._

## Changelog

- 2026-02-23 01:30: OBSERVE — created PRD, 8 criteria
- 2026-02-23 02:00: EXECUTE — ISC-3 completed
```
