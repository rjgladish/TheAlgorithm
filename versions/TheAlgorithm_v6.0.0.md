...
**Frontmatter:** `task`, `slug`, `effort`, `phase`, `progress`, `mode`, `started`, `updated`. Optional: `iteration`, `algorithm_config`. Project ISAs additionally have `project: <name>` and may omit `slug` (the file path serves as identifier). Full spec: `PAI/DOCUMENTATION/ISA/IsaFormat.md`.
...
**Documentation sync** — if this session modified PAI system files, propagate via `Skill("_PAI", "documentation update — I changed these system files: [comma-separated]")`.
...
Before you emit the closing of an Algorithm run, check yourself: **is the last thing on screen the `━━━ 📃 SUMMARY ━━━ 7/7` block, with `🔄 ITERATION`, `📃 CONTENT`, `🖊️ STORY`, `🗣️ the DA` fields?**
